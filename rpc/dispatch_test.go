...
func dispatchTestCallWithContext(t *testing.T, ctx context.Context) (dispatcher, *callContainer, chan error) {
	...
	d := newDispatch(dispatchOut, calls, logFactory.NewLog(nil))
		...
		return d.Call(ctx, "abc.hello", new(interface{}), new(interface{}), nil, nil, nil)
	})
...
func TestDispatchCallAfterClose(t *testing.T) {
	...
	d.Close()
		...
		return d.Call(context.Background(), "whatever", new(interface{}), new(interface{}), nil, nil, nil)
	})
	err = <-done
...
func TestDispatchCancelEndToEnd(t *testing.T) {
	...
	ch := make(chan error)
	go func() {
		err := d.Call(ctx1, "abc.hello", nil, new(interface{}), nil, nil, nil)
		ch <- err
	}()
